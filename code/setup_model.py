...
# File description:
#
# Sets up the SG model developed by Gancarz & Grossberg (1998) in PyNEST for
...
delay   : conductance delay (in ms); no delay if equal to time step
tau     : time constant (in ms)
sigma   : scaling factor of additive noise
'''
dt = 0.05
delay = 0.05
tau = 50.00
...
neuron parameters
-----------------
alpha			 : upper limit for a piecewise linear unit
                   (2nd threshold for threshold_lin_rate)
g_in			 : gain in multiplicative coupling for inhibitory input
lambda			 : passive decay rate
linear_summation : if false, gain function is applied prior to input summation;
                   if true, it is applied afterwards
mult_coupling    : if false, neuron does not exhibit multiplicative coupling;
                   if true, it does
rectify_output	 : if false, rates updated according to numerical integration;
                   if true, rates are rectified after each update
theta 			 : lower limit for threshold (piecewise) linear unit
                   (1st thresold for threshold_lin_rate)
