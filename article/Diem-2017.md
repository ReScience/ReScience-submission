...
![Input functions for the NVU model. K+ (blue) refers to potassium and Glu (orange) refers to glutamate released into the synaptic space as a consequence of synaptic activity by the neuron. Potassium levels are given in units of $\mu$M while the glutamate value actually refers to the ratio of bound and unbound glutamate receptors (equation (3) [@Witthoft2012]) and is therefore unitless. Maximum values were found by trial and error such that potassium and IP\textsubscript{3} in Figure @fig:fig1 here match Figure 5 in the original publication.](figures/input.png){#fig:input}

The input to the model is not explictly provided in the original publication, only the statement ``We simulated neural-induced vasodilation by representing the total synaptic activity in the astrocyte domain as a uniform, continuous smooth pulse of glutamate ($J_{K_s}$) and of synaptic potassium ([K\textsuperscript{+}]\textsubscript{s}).'' is provided [@Witthoft2012]. Upon correspondence with the authors both glutamate and synaptic potassium were modelled as a linear increase over time up to a maximum value of 0.55 uM/s for K\textsuperscript{+} and 0.5 for the ratio of bound/unbound Glu receptors, which was sustained for a time period of 18 s, followed by a linear decrease. The functions for the input are provided in the code of this implementation (Figure @fig:input). Maximum values were found by trial and error such that potassium and IP\textsubscript{3} in Figure @fig:fig1 here match Figure 5 in the origial publication.
...
![Astrocyte membrane potential due to quick manual inflation of the artery. The experimental data describing the artery stretch was obtained from [@Witthoft2012] and originates from [@Cao2011]. The astrocyte membrane potential was expected to range from -80 mV to -40 mV and its decay was expected to be slower. Simulation tolerance values atol=1e-7, rtol=1e-8.](figures/Vk_inflation.png){#fig:vk_inflation}

The original publication contains results to validate their TRPV model equations (Figure 3 [@Witthoft2012]). They obtained data of the astrocyte membrane potential (Vk) of an artery being inflated by 20-40\% of its original size in quick bursts [@Cao2011]. The data of the stretching of the vessel was extracted from [@Witthoft2012] and supplied as an input (variable $x$) to the NVU model (main_trpv.py). Figure @fig:vk_inflation shows the equivalent of Figure 3 in the original publication. The figures compare well to some degree, however, the astrocyte membrane potential appears to decay too quickly in this simulation. Additionally, the value range does not match, which should be -72 mV to -40 mV in the original publication, while it is approximately -60 mV to 50 mV here. Tests to vary the parameters of equation (23), which governs the change in perivascular calcium, did not yield further improvements.

![Effect of the vasodilatory drug pinacidil on the results. Radial strain appears to be a good match, although its increase starts later compared to the results in the original publication. The values for perivascular calcium are much higher than in the original publication, but the reason for this could not be found.](figures/pinacidil_2000uM.png){#fig:pinacidil}
...
The results of this modification (main_k.py) are shown in @fig:pinacidil. Radial strain appears to be a good match, although its increase starts later compared to the results in the original publication. The values for perivascular calcium are much higher than in the original publication, but the reason for this could not be found.
