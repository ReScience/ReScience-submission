...
def nvu(t, y, Jrho_IN, x_rel, Vk_f):
    potassium_s = y[0]
    ip3 = y[1]
    calcium_a = y[2]
    h = y[3]
    ...
    return [potassium_s_dt, ip3_dt, calcium_a_dt, h_dt, ss_dt, eet_dt, nbk_dt,
            potassium_p_dt, calcium_p_dt, k_dt, Vm_dt, n_dt, x_dt,
...
def init():
    ...
    return [potassium_s, ip3, calcium_a, h, ss, eet, nbk, potassium_p,
...
def main(fig_dims):
    ...
    Vk = np.loadtxt("../data/vk.csv", delimiter=',')
    Vk_f = interp1d(Vk[:,0], Vk[:,1]*mV, bounds_error=False, fill_value=Vk[-1,1]*mV)
    ...
    ode15s = ode(nvu)
    ode15s.set_f_params(Jrho_IN, x_rel, Vk_f)
    ...
    # right side
    axarr[0, 1].plot(t, Vk_f(t)/mV, label="", lw=2)
    axarr[0, 1].set_ylabel("Vk (mV)")
    axarr[1, 1].plot(t, sol[:,7]/mM, label="", lw=2)
    axarr[1, 1].set_ylabel("K+ pvs (mM)")
    axarr[2, 1].plot(t, sol[:,13]/uM, label="", lw=2)
