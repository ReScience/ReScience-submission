...
To compare the distributions of firing rates and CVs obtained from simulations in Brian 2 and NEST we use the Kolmogorov-Smirnov statistical test (KS-statistics). To apply this non-parametric test, cumulated histograms were constructed with bins chosen by the Doane method \cite{doane1976}. 
...
In order to compare the Brian 2 with the NEST implementations of the Potjans-Diesmann model, we used the available pyNEST code \cite{potjans2014pyNEST} to run NEST simulations of the model with the same parameters given in Methods. The comparisons were made in terms of the cumulative distributions of the CVs of ISIs and the firing rates of the eight cell populations in the network (plots not shown here but as in Figs.~\ref{fig:fig3} and~\ref{fig:fig4} below). The two-sample Kolmogorov-Smirnov test pointed that no difference was found between NEST and Brian 2 simulations.
...
When comparing the network constructed in Brian 2 with equation~(\ref{Eq:Ksyn_alt}) and the one constructed in NEST, all layer-specific comparisons yielded KS-statistics lower than 1.14, indicating no significant differences (for an error $\alpha = 0.01$) between the firing rate and CV distributions. However, when comparing the mean firing rates and their standard deviations, in general, slightly differences were found being major for L5e cells, which in the replicated Brian 2 version using equation~(\ref{Eq:Ksyn_alt}) have mean firing rate 12.2 $\pm$ 6.1 Hz, while using equation~(\ref{Eq:Ksyn}) their mean firing rate is 7.8 $\pm$ 4.8 Hz and in the original version their mean firing rate is 7.8 $\pm$ 5.1 Hz.
