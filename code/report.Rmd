...
Now select only the date range used in the Nature paper. From the supplment *The analysis in Benincà et al. (Nature 2008) covered all data from 16/06/1991 until 20/10/1997*. 
```{r}
...
# next two lines make variables for testing
#x <- trimmed_raw %>% filter(variable=="Cyclopoids") %$% Day.number
#y <- trimmed_raw %>% filter(variable=="Cyclopoids") %$% value
...
qplot(x=as.vector(original.cors), y=as.vector(cor.coefs), ylim = c(-0.4,0.4), xlim=c(-0.4, 0.4),
      xlab="Correlations in original article", ylab="Correlations calculated in this reproduction") + geom_abline(intercept=0,slope=1, colour="red")
...
It seems that this must have been done on data without zeros removed...
...
g1 <- ggplot(mt, aes(x=as.numeric(Day.number), y=sqrt(value))) +
  ...
  scale_colour_manual(values = species.colour.mapping)
g2 <- geom_line(data=from.steve, aes(x=Day.number, y=sqrt(value_steve)), col="red")
g1 + g2
...
qplot(data=ff, x=log10(value_steve), y=log10(value)) +
  xlab("Values from Ellner (log 10 transformed)") +
  ylab("Reproduced values (log 10 transformed)") +
