...
class Priors_model():
    ...
    def temporalcoherence_loss(self, args):
        ...
        return (self.norm2(delta_st1) + self.norm2(delta_st2))/2.0
    ...
    def causality_loss(self, args):
        ...
        return K.switch( same_a_diff_r, K.exp( -self.norm2(s_t2-s_t1) ), K.zeros_like(same_a_diff_r) )/K.mean(same_a_diff_r, axis=0)
    ...
    def repeatability_loss(self, args):
        ...
        # repeatability_loss
        #   Note the K.mean(same_action, axis=0) normalization, because this is a conditional expectation on same_action
        return K.switch( same_action, Multiply()([K.exp(-self.norm2( s_t2-s_t1)), self.norm2(delta_st1-delta_st2)]), K.zeros_like(same_action)
                                             )/K.mean(same_action, axis=0)
    ...
    def training_generator(observations, actions, rewards, episode_starts, batch_size):
        ...
        while True:
            ...
            # unused here because this is unsupervised learning, yet it is needed. 4 arrays for the four losses
            Y = [np.zeros([batch_size,1]) for i in range(4)]
            yield X,Y
