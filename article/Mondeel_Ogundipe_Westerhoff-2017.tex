...
phenylalanine as biomarkers and gets the directionality of the change in
their concentrations right. In contrast, the prediction based on Recon
2.2 lacks tyrosine as a biomarker.
...
taking the union of their predicted biomarkers. When a biomarker was
subject to qualitatively different predictions between the
enzyme-catalyzed reactions encoded by the gene, a majority rule was
applied. We will refer to this as the asynchronous setting. Blocking all
affected reactions simultaneously could yield different predictions. We
will refer to this as the synchronous setting. We added the option to
choose between the asynchronous and synchronous setting to our
implementation.
...
The original publication's Figure 1 exemplified the biomarker prediction
method for an illustrative network. It is reproduced here as Figure
~\ref{fig:Shlomi2009_figure_1}. The simple nature of the example serves
to explain the reasoning behind the method and additionally helped us
validate our implementation of the method. This repository provides an
SBML file of the network, the Jupyter notebook that generates it and a
Jupyter notebook reproducing the biomarker prediction.
...
Shlomi et al. provide the a biomarker prediction for this network
graphically in their Figure 1. We here do so numerically and a Jupyter
notebook reproducing the biomarker prediction accompanies this paper. In
our analysis all exchange reactions were given inward bounds of \(-10\)
and outward bounds of \(+1000\) so that the internal reaction bounds (of
