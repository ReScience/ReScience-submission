...
\begin{figure}[htbp]
\centering
\includegraphics{figures/dynamics.pdf}
...
\caption{\label{fig:LE_comparison}Comparison of Lyapunov exponents in
the original article and this reproduction.}
\end{figure}
...
largely identical. Quantitative differences may have resulted from
...
was largely inspired by the TISEAN project. There may also have been
some difference in data used for specific analyses, e.g., data with
zeros removed or not, as it was not always possible to be sure the
reproduction used exactly the same data.
