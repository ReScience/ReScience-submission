...
# Fig 5a)
fig5a = plot(sim1[:,1], sim1[:,2], label="Hosts", frame=:origin, lw=2, leg=false, ylims=(0, 150), xlims=(0,50))
plot!(fig5a, sim1[:,1], sim1[:,3], label="Parasites", lw=2)
ylabel!(fig5a, "Population size")
annotate!(fig5a, generate_legend_position((0,50),(0,150))..., text("(a)"))
...
annotate!(fig5d, generate_legend_position((-1,2.5),(0,0.5))..., text("(d)"))
...
# Fig 5b)
fig5b = plot(sim2[:,1], sim2[:,2], label="Hosts", frame=:origin, lw=2, leg=false, ylims=(0, 150), xlims=(0,50))
plot!(fig5b, sim2[:,1],sim2[:,3], label="Parasites", lw=2)
xlabel!(fig5b, "Generation")
annotate!(fig5b, generate_legend_position((0,50),(0,150))..., text("(b)"))
...
annotate!(fig5e, generate_legend_position((1.5,2.5),(0,0.5))..., text("(e)"))
...
# Fig 5c)
fig5c = plot(sim3[:,1],sim3[:,2], label="Hosts", frame=:origin, lw=2, ylims=(0, 150), xlims=(0,50))
plot!(fig5c, sim3[:,1],sim3[:,3], label="Parasites", lw=2)
annotate!(fig5c, generate_legend_position((0,50),(0,150))..., text("(c)"))
...
fig5f = scatter(log10.(sim3[:,2]), kval3,
   ...
   xlims=(1.0, 2.0),
   ylims=(0, 0.5))
annotate!(fig5f, generate_legend_position((1,2),(0,0.5))..., text("(f)"))
