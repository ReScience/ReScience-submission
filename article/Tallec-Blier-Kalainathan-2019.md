...
We reproduced the authors results on the CarRacing environment. We only used
the paper description, and took exactly the same hyperparameters as the
original paper. We did not use any original sources, and we did not contact the
authors.

### Training in details
#### Data generation
The original paper started by generating rollouts using a random policy in
interaction with the environment. The policy was not specified. In our
experiments we tested two types of policies. The first policy generates
independant centered gaussian actions at each step. The second policy 
...
$$a^k_{t+1} = a^k_{t} + 1 / \sqrt{50} \varepsilon^k_t$$
where the $\varepsilon$'s are i.i.d. gaussian random variables. The data
samples generated by the first policies lack diversity, and the car only moves
in a very restricted area of the track. The samples generated by the second
policies, on the other are much more diverse, and were consequently used for
...
#### Mixture Density Recurrent Neural Network (MDRNN) training
Similarily the MDRNN is trained following the procedure in the original paper.
...
RMSprop @rmsprop is used to optimize the MDRNN, with a learning rate $10^{-3}$,
...
Evolution Strategy @hansen. As in the original paper, the controller is a linear
network that takes as inputs the concatenation of the output of the VAE and the hidden
layer of the MDRNN. For a set of parameters of the controller, the loss
function is obtained by averaging the returns from $16$ rollouts of length at most
$1000$. The population size used for CMAES is $64$, and an initial standard deviation
of $0.1$ is used. Rollouts are executed parallely, on $64$ threads, with models sharing
$8$ V100 GPUs.
...
implementation reached a best score of ???, below the 906 reported in the
paper, but much better than the second best benchmark reported which is around
780. Results are further detailled in the figures ...

## Additional experiments

We wanted to test the impact of the MDRNN on the results. Indeed, we observed
...
their results with a model without the MDRNN. They obtained the following
scores:
...
controller, but with an untrained MDRNN instead of the trained one (we kept it
at its random initialization values). Similarily, we tried training a controller
with both an untrained MDRNN and an untrained VAE. 
...
controllers with both untrained VAE and MDRNN achieves very low performance.
