...
partitioning method can influence the error rates. Verbyla and Litaitis
briefly reviewed a range of partitioning methods in their
assessment of resampling methods for evaluating classification
accuracy.” The work of Verbyla and Litvatis @verbyla-1989 remains the
...
error rate used by Verbyla and Litvatis, as when
...
randomly partitions the dataset into training and testing subsets. The
proportion $p$ of data ‘held-out’ from the dataset $d$ forms a testing
...
## $K$-fold cross-validation and the jackknife

The $K$-fold cross-validation method begins by randomly partitioning the
...
called the jackknife (or leave-one-out cross-validation).

## Bootstrap cross-validation

This methods is based upon a set $B$ of bootstrap sample datasets $b$,
...
and the remaining 0.328 of $d$ occurring in testing set $\{d - b\}$
...
by Verbyla and Litvatis that we are trying to replicate.

# Computational experiment replication

Verbyla and Litvatis based their computational experiment
...
and Litvatis state that the “ten predictor variables were
...
standard normal distribution of $\mu$ = 0 and $\sigma$ = 1. Each
resampling method was then applied to the same iterations of the random
dataset.
...
Verbyla and Litvatis — remembering that $K=n$ is
...
specified in the example code provided by Verbyla and Litvatis.

## Hold-out cross-validation

We included three cases of hold-out cross-validation as while Verbyla
and Litvatis stated that for this method the “the estimate
...
addition we felt there was inconsistency in the two citations given with
reference to this method, as one referred to hold-out cross-validation
with $H=1$ @lachenbruch-1968, while a second referred hold-out
...
Litvatis as a “smoothed frequency distribution” but the
...
prediction error of 0.5 also marked.](resampling-results.png){#fig:resampling-results}
...
different to that of Verbyla and Litvatis, we would
...
Litvatis in advocating the use of the bootstrap, as it
