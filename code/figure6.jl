...
# Fig 5a)
fig6a = plot(sim1[:,1], sim1[:,2], label="Hosts", frame=:origin, lw=3, leg=false, ylims=(0, 60))
...
annotate!(fig6d, -0.25, 0.55, text("(d)"))
...
# Fig 5b)
fig6b = plot(sim2[:,1], sim2[:,2], label="Hosts", frame=:origin, lw=3, leg=false, ylims=(0, 60))
...
annotate!(fig6e, -0.25, 0.55, text("(e)"))
...
# Fig 5c)
fig6c = plot(sim3[:,1],sim3[:,2], label="Hosts", frame=:origin, lw=3, ylims=(0, 60))
...
fig6f = scatter(log10.(sim3[:,2]), kval3,
   ...
   xlims=(0.5, 1.5),
   ylims=(0, 0.6))
annotate!(fig6f, -0.25, 0.55, text("(f)"))
