...
We note that in early versions of this reproduction we were utilising a medium that allowed influx of $-1$ for all medium components, as opposed to $-10$ now. It turns out that a setting of $-1$ does accurately reproduce the results for Figure 2, but not for Figure 3. For the CBS IEM the WT interval would correspond to $[-1,-1]$ and would therefore be a point instead of a wide interval as Shlomi et al. draw it. We included this simulation in the last cell of the notebook *Reproduce_figure_2_and_3_Shlomi2009.ipynb* This is another example of the inherent sensitivity of the approach to various settings in the algorithm.
