...
#dataset_to_use <- "reproduced" # 0.03668501
dataset_to_use <- "from_steve" # 0.08415112

if(dataset_to_use=="from_steve") {
  X=read.csv("../../data/original/interp_short_allsystem_original.csv")
}

if(dataset_to_use =="reproduced") {
  X=read.csv("../../data/reproduction/Repro_data_for_gam_test.csv", row.names=1)
  names(X) <- name_table$steve_names[match(names(X), name_table$repro_names)]
}
...
### From here on is code pretty much identical to what Steve sent
...
## next line just to output some data to put into Owen's function
#write.csv(Z, "~/Desktop/Steve_data_for_gam_test_transformed.csv")
