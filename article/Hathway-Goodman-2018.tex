...
replication implementation behaves the same as in the original study as.
...
paper. \textbf{B}) The postsynpatic potential in the replication
...
neighbor rule (RNN) cite\{Morrison2008\} - a more restrictive version of
the nearest neighbor (NN) STDP rule. In the standard NN rule, every
...
depressions strictly alternate (due to neurons being forced to spike at
least once every 50 ms, see \cite{Masq2008}).

A visualisation of three STDP rules is shown in Figure ~\ref{fig:stdp}:
the RNN, the standard NN and the all-to-all (ATA) rule, in which all
pairs of spikes are considered to calculate the weight change. A case is
...
no visible net change under RNN (Figure ~\ref{fig:stdp}\textbf{C}).
...
(\textbf{C}), there are fewer changes and - in this example - no visible
net weight change.}\label{fig:stdp}
\end{figure}
...
behaviour: latency development (Figure ~\ref{fig:lat}), pattern
specificity after convergence (Figure ~\ref{fig:conv}), and robustness
(Figure ~\ref{fig:para}). In addition, we comment on some implementation
...
both implementations (see Figure ~\ref{fig:lat}). The time until a
stable state arises is longer in the replication implementation (1400
...
start of the pattern is random at first, but becomes selective after a
few hundred discharges. From then on the postsynaptic neuron always
spikes only within the pattern (no spikes at latency = 0). The number of
discharges until this happens is smaller in the original paper than in
the replication implementation.}\label{fig:lat}
\end{figure}
...
the pattern are depressed nearly completely (see Figure
~\ref{fig:conv}). Neurons that spike at the beginning of the pattern are
potentiated, causing the postsynaptic neuron to spike at a low latency.

\begin{figure}
\centering
\includegraphics{figures/fig4_convergence.pdf}
\caption{Weights at converged state. The weights in the converged state
look very similar in the the original study (\textbf{A)}) and the
replication implementation (\textbf{B)}). Weights from neurons not
involved in the pattern are close to 0 (neurons 1000-1999), whereas
weights from some neurons involved in the pattern (neurons 0-999) are
close to 1 (maximum value). The weights of neurons that spike at the
beginning of the pattern are nearly all close to 1 and their added
postsynaptic potentials cause a spike at the beginning of the pattern
(blue rectangle).}\label{fig:conv}
\end{figure}
...
when subjected to different amounts of jitter (Figure ~\ref{fig:para}
\textbf{B}), various proportions of pattern neuron (Figure
~\ref{fig:para} \textbf{C}), different initial weights (Figure
~\ref{fig:para} \textbf{D}) and when a percentage of spikes within the
pattern are deleted (Figure ~\ref{fig:para} \textbf{E}).
...
steps: 10\(^{-6}\) s (solid black lines) and 10\(^{-4}\) s (dashed black
...
At a high pattern repetition frequency of 0.5 (Figure ~\ref{fig:para}
...
with any other than the reduced NN learning rule - the input neurons
will experience a decrease in weight a lot more often (every time an
input neuron spikes) than an increase in weight (every time the
postsynaptic neuron spikes). This leads to a strong overall depression
of the synaptic weights in the first few seconds of the simulation. With
the parameters specified in \textcite{Masq2008} and an ATA or a
conventional NN learning rule, the output neuron stops firing after a
few seconds because the output neuron voltage does not reach the voltage
threshold necessary to evoke an output neuron spike anymore (see Figure
~\ref{fig:meanw}\}). In the case of the standard parameters, the output
neuron stops firing after less than one second. When no output neuron
spikes occur, learning cannot take place and no specificity can emerge.
...
a\(_{+}\) substantially) it is possible to achieve a behaviour that
...
learning rate by a factor of 5 and increasing the value of a\(_{+}\)
(the maximum weight increase) to double the value of a\(_{-}\) (the
...
(due to the increased a\(_{+}\)). In such runs, the output neuron will
reach specificity and trace back through the latency and then starts
firing outside of the pattern again. This system is not stable since the
ATA rule leads to ``too much learning'' as the synaptic weights change
after every single spike. A further reduction in the learning rule will
not results in pattern specificity at all.

The standard NN learning rule, although exhibiting slower learning when
compared to the ATA rule, is unable to accomplish any pattern finding
because the limitation of potentiation to a certain maximum value means
that depression will always outweigh potentiation as it occurs far more
often.

\subsubsection{EPSP shape}\label{epsp-shape}

The kernels used in the original study simulate a gradual increase of
the postsynaptic neuron voltage as can be seen in Figure ~\ref{fig:pot}.
...
voltage does not lead to stable finding. This might have to to with the
fact that the kernel shape and the immediate increase exhibit slightly
different spike times as seen in Figure ~\ref{fig:epsps}.
